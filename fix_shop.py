...
# The header nodes are 124:471, 124:472, 124:473, 124:474, 124:479 (which contains 480-485)
# In React output, these are top-level children of the root div.
# Let's just remove them by using regex on the React code.
# The safest way is to use a simple string replacement on the extracted JSX.
...
# We can remove the header divs by matching the specific data-node-ids and their content.
# Actually, since we know exactly how they look, we can just replace them.
# Safely remove header nodes (124:471 to 124:479) which are contiguous before 124:605
lines = jsx.split('\n')
new_lines = []
skip = False
for line in lines:
    if 'data-node-id="124:471"' in line:
        skip = True
...
def replace_tailwind(m):
    ...
    for c in classes:
        ...
        if c.startswith('w-['):
            ...
            val = c[6:-1]
            if val.startswith('#') or val.startswith('rgba'): styles.append(f'color: {val};')
            elif val.endswith('px'): styles.append(f'font-size: {float(val[:-2])/12.8}vw;')
            else: styles.append(f'font-size: {val};')
        elif c.startswith('bg-['):
            val = c[4:-1]
            if val.startswith('#') or val.startswith('rgba'): styles.append(f'background-color: {val};')
...
def replace_jsx_style(m):
    ...
    if "backgroundImage" in inner:
        inner = "background-image: linear-gradient(90deg, rgb(248, 244, 231) 0%, rgb(248, 244, 231) 100%), linear-gradient(90deg, rgb(255, 255, 255) 0%, rgb(255, 255, 255) 100%);"
    elif "maskImage" in inner:
        var_match = re.search(r'\$\{([^}]+)\}', inner)
        if var_match:
            var_name = var_match.group(1)
            path = img_defs.get(var_name, "assets/unknown.svg")
            inner = f"mask-image: url('{path}'); -webkit-mask-image: url('{path}');"
        else:
            inner = ""
...
# Read index.html to extract its header navigation
with open('index.html', 'r', encoding='utf-8') as f:
    index_html = f.read()

header_match = re.search(r'<!-- Top horizontal line -->.*?<!-- Logo text top left -->.*?</a>', index_html, re.DOTALL)
index_header = header_match.group(0) if header_match else ''
# We need to change the SHOP link to be active (font-weight: 500) and ABOUT to index.html#everyday in the injected header
# In index.html: <a href="shop.html" class="nav-link nav-link-text"...>SHOP</a>
# Properly inject font-weight: 600 without creating duplicate style attributes
index_header = re.sub(r'(href="shop\.html"[^>]*?style=")', r'\1font-weight: 600; ', index_header)
index_header = index_header.replace('href="#everyday"', 'href="index.html#everyday"')
...
# The original JSX already has `<div className="relative size-full"...>` which we will just put inside `figma-container`
# Wait, `html` is just the inner div. If we wrap it in `figma-container` and put the `index_header` inside `figma-container`, it will overlay perfectly because `index_header` elements use `.abs` which is `position: absolute;` and `z-index` from `style.css`!
