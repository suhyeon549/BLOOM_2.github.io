...
urls = list(set(re.findall(r'http://localhost:3845/assets/[a-f0-9]+\.(?:png|svg)', text)))
...
for url in urls:
    ...
    # The output.txt assigns them to variables, we need to map the variable name
...
def replace_jsx_style(m):
    inner = m.group(1)
    # Fix gradients and mask images safely
    if "backgroundImage" in inner:
        # manual replace for the gradient
...
texts.forEach(p => {{
  if(p.textContent.trim() === 'ABOUT') {{
    p.outerHTML = '<a href="index.html#everyday" style="' + p.getAttribute('style') + '; text-decoration: none; cursor: pointer; color: inherit; z-index: 9999;">ABOUT</a>';
  }}
  if(p.textContent.trim() === 'TEAM') {{
    p.outerHTML = '<a href="team.html" style="' + p.getAttribute('style') + '; text-decoration: none; cursor: pointer; color: inherit; z-index: 9999;">TEAM</a>';
  }}
